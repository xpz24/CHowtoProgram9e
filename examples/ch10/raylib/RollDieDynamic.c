    ...
    if (!simulationOver) {
        ...
        if (!pause) {
            ...
            for (int i = 1; i <= FACES; ++i) {
                ...
                bars[i].rect.width = (int)(counts[i] * 1.0 / counts[index] * barMaxWidth);
            }
        }
...
void DrawFrame(void) {
    ...
    for (int i = 1; i <= FACES; ++i) {
        ...
        DrawText(FormatText("%d", counts[i]), x + 70, y + dy * i, fontSize, BLACK);
        DrawText(FormatText("%.2f", counts[i] * 100.0 / totalRolls), x + 240, y + dy * i, fontSize,
                 BLACK);
        DrawRectangleRec(bars[i].rect, bars[i].color);
    }

    // draw total number of dice rolled so far
    DrawText(FormatText("%d", totalRolls), x + 70, y + dy * 7, fontSize, BLACK);
