...
void InitGame(void) {
    ...
    for (int i = 0; i < TARGETS; ++i) {
        ...
        double velocity = GetRandomValue(screenHeight * targetMinSpeedPercent,
...
void UpdateGame(void) {
    if (!gameOver) {
        ...
        if (!pause) {
            ...
            if (!cannon.ball.onScreen && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                ++shotsFired;
                ...
                if (cannon.ball.position.x > screenWidth || cannon.ball.position.x < 0 ||
                    cannon.ball.position.y > screenHeight || cannon.ball.position.y < 0) {
                    cannon.ball.onScreen = false;
                }

                // check for Cannonball/Blocker collision
                else if (CheckCollisionCircleRec(cannon.ball.position, cannon.ball.radius,
                                                 blocker.rect)) {
                ...
                // check for Cannonball/Target collision
                    ...
                    for (int i = 0; i < TARGETS; ++i) {
                        if (!targets[i].destroyed &&
                            CheckCollisionCircleRec(cannon.ball.position, cannon.ball.radius,
                                                    targets[i].rect)) {
            ...
            if (blocker.rect.y <= 0 || blocker.rect.y + blocker.rect.height >= screenHeight) {
                blocker.dy *= -1;
            }
            ...
            for (int i = 0; i < TARGETS; ++i) {
                ...
                if (targets[i].rect.y <= 0 ||
                    targets[i].rect.y + targets[i].rect.height >= screenHeight) {
...
void DrawGame(void) {
    ...
    DrawLineEx(cannon.barrelStart, cannon.barrelEnd, cannon.barrelThickness, cannon.color);
    ...
    if (cannon.ball.onScreen) {
        DrawCircleV(cannon.ball.position, cannon.ball.radius, cannon.ball.color);
    }
    ...
    DrawText(TextFormat("TIME REMAINING: %.1f", timeLeft), 15, 15, fontSize, DARKGRAY);

    // if game over, display final stats and instructions to play again
    if (gameOver) {
        DrawText(targetsHit == TARGETS ? "YOU WIN!" : "YOU LOSE.", 15,
                 screenHeight - 15 - (fontSize + 5) * 5, fontSize, DARKGREEN);
        DrawText(TextFormat("SHOTS: %d", shotsFired), 15, screenHeight - 15 - (fontSize + 5) * 4,
                 fontSize, DARKGREEN);
        DrawText(TextFormat("HITS: %d", targetsHit), 15, screenHeight - 15 - (fontSize + 5) * 3,
                 fontSize, DARKGREEN);
        DrawText(TextFormat("ELAPSED TIME: %.1f SECONDS", elapsedTime), 15,
                 screenHeight - 15 - (fontSize + 5) * 2, fontSize, DARKGREEN);
        DrawText("PRESS ENTER TO PLAY AGAIN", 15, screenHeight - 15 - (fontSize + 5), fontSize,
                 DARKGREEN);
    }
...
void alignBarrel(double barrelAngle) {
    ...
    cannon.barrelEnd.y = -cannon.barrelLength * cos(barrelAngle) + screenHeight / 2;
}
...
void alignAndFireCannonball(Vector2 point) {
    ...
    cannon.ball.dx = cannonballSpeedPercent * screenWidth * sin(cannon.barrelAngle);
    cannon.ball.dy = cannonballSpeedPercent * screenWidth * -cos(cannon.barrelAngle);
