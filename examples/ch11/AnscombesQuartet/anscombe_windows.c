...
void performRegression(double x[], double y[], double *intercept, double *slope) {
    ...
    */
    gsl_fit_linear(x, 1, y, 1, SAMPLES, intercept, slope, &cov00, &cov01, &cov11, &chi_squared);
}

// read Anscombe's Quartet CSV data into program
void readAnscombesQuartetData(double x1[], double y1[], double x2[], double y2[], double x3[],
    ...
    for (int i = 1; i <= DATASETS; ++i) {
        ...
        /* gnuplot's "plot" command creates a plot. The following is a sample
           plot command sent to gnuplot when the fprintf below executes:
              ...
              plot 'anscombe.csv' using 1:2 with points pointtype 7, 0.5*x+3.0
        ...
        */
        fprintf(pipe,
                "plot 'anscombe.csv' using %d:%d with points "
