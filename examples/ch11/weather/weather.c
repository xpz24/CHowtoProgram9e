...
int main(int argc, char *argv[]) {
    ...
    sprintf(queryURL, "%s%s&units=metric&appid=%s", apiURL, argv[1], argv[2]);
    //   printf("URL: %s\n", queryURL);
    ...
    */
    ...
    if (parsedJson != NULL) {
        ...
        const cJSON *cityName = cJSON_GetObjectItemCaseSensitive(parsedJson, "name");
        ...
        const cJSON *mainData = cJSON_GetObjectItemCaseSensitive(parsedJson, "main");
        ...
        if (cJSON_IsObject(mainData)) {
            ...
            const cJSON *tempCurrent = cJSON_GetObjectItemCaseSensitive(mainData, "temp");
            const cJSON *feelsLike = cJSON_GetObjectItemCaseSensitive(mainData, "feels_like");
            const cJSON *pressure = cJSON_GetObjectItemCaseSensitive(mainData, "pressure");
            const cJSON *humidity = cJSON_GetObjectItemCaseSensitive(mainData, "humidity");
        ...
        const cJSON *weather = cJSON_GetObjectItemCaseSensitive(parsedJson, "weather");
        ...
        cJSON_ArrayForEach(day, weather) {
            ...
            cJSON *description = cJSON_GetObjectItemCaseSensitive(day, "description");
